...
def fetch_candidate_chunks(client, db_name, collections, keywords, limit_per_keyword=300):
    db = client[db_name]
    candidates = []
...
def generate_qa_pair(client_openai, context):
    ...
    91:     ข้อกำหนดสำคัญ (Critical Requirement):
    92:     1. **ต้อง** สมมติ "วันเดือนปีเกิด" ใส่ลงในคำถามในรูปแบบ **DD/MM/YYYY** (เช่น "คนเกิดวันที่ 15/04/1990...") เพื่อจำลองผู้ใช้งานจริง
    93:     2. หากบริบทกล่าวถึง "ราศี" ใด ให้เลือกวันเกิดที่อยู่ในช่วงราศีนั้น (เช่น พฤษภ -> 15/05/xxxx) เพื่อความสมเหตุสมผล
    94:     3. **คำตอบ (Answer)** ต้องสร้างจาก **ข้อมูลในบริบทเท่านั้น** (ห้ามนำความรู้นอกเหนือจากบริบทมาตอบ แม้ความรู้นั้นจะถูกตามหลักโหราศาสตร์ก็ตาม)
    95:     4. **การเลือกใช้คำ (Vocabulary matches context):** หากในบริบทมีคำศัพท์เฉพาะ (เช่น "สีกาลกิณี", "เรือนชะตา", "มุมโยค") ให้ใช้คำศัพท์นั้นใน "คำถาม" ด้วย ห้ามใช้คำพ้องความหมายหรือคำอื่นแทน (เช่น ห้ามใช้ "สีไม่มงคล" หรือ "สีอัปมงคล" ถ้าในบริบทเขียนว่า "สีกาลกิณี") เพื่อให้มั่นใจว่าคำถามสอดคล้องกับเอกสาร
    96:     5. **ห้ามถามเกี่ยวกับวันในสัปดาห์ (No Day of Week):** ห้ามสร้างคำถามหรือคำตอบที่อิงกับ "วันในสัปดาห์" (เช่น จันทร์-อาทิตย์, วันพฤหัสบดี) โดยเด็ดขาด ให้เน้นคำถามที่เกี่ยวกับ **"ราศี" (Zodiac)**, **"ดาวเคราะห์" (Planets)**, หรือ **"มุมสัมพันธ์" (Aspects)** เท่านั้น หากบริบทมีแค่เรื่องวัน ให้ **ข้าม** บริบทนั้นไปเลยหรือไม่ต้องกล่าวถึงวัน
    
    รูปแบบภาษา:
    - คำถาม: ภาษาไทย มีวันเกิดระบุชัดเจน และใช้คำศัพท์ตรงกับบริบท
    - คำตอบ: ภาษาไทย ตอบตามเนื้อหาในบริบทอย่างเคร่งครัด กระชับ และตรงประเด็น
