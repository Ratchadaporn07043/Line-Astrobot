...
class BirthDateParser:
    ...
    def detect_zodiac_sign_in_message(self, text: str) -> dict:
        """
        ตรวจจับชื่อราศีในข้อความ
        
        Args:
            text (str): ข้อความที่ต้องการตรวจสอบ
            
        Returns:
            dict: ข้อมูลราศีที่พบ {'sign': 'ชื่อราศี', 'element': 'ธาตุ', 'quality': 'คุณภาพ', 'english_name': 'ชื่ออังกฤษ'} หรือ None
        """
        ...
        for sign_key, sign_info in zodiac_data.items():
            for keyword in sign_info['keywords']:
                # ใช้ regex เพื่อตรวจสอบว่าพบคำเต็มๆ (ไม่ใช่ส่วนหนึ่งของคำอื่น)
                # เช่น "กันย์" ไม่ควร match กับ "กันยายน"
                pattern = r'\b' + re.escape(keyword) + r'\b'
                if re.search(pattern, text_lower):
                    logger.info(f"พบชื่อราศีในข้อความ: {sign_info['name']} (keyword: {keyword})")
                    return {
                        'sign': sign_info['name'],
...
def detect_zodiac_sign_in_message(message: str) -> dict:
    """
    ฟังก์ชันง่ายๆ สำหรับตรวจจับชื่อราศีในข้อความ
    
    Args:
        message (str): ข้อความจากผู้ใช้
        
    Returns:
        dict: ข้อมูลราศีที่พบ หรือ None
    """
...
def create_birth_chart_query(chart_info: dict, birth_info: dict) -> str:
    ...
    # 🆕 สร้าง query ที่สั้นและใช้ชื่อราศีเป็นหลัก เพื่อให้ค้นหาในฐานข้อมูลได้ดีขึ้น
    zodiac_sign = chart_info['zodiac_sign']
    
    # สร้าง query แบบสั้นที่ใช้ชื่อราศีเป็นหลัก
    query = f"ทำนายดวงกำเนิดราศี{zodiac_sign} ลักษณะนิสัย การงาน การเงิน ความรัก"
    
    # เพิ่มข้อมูลเพิ่มเติมถ้ามี (แต่ไม่ทำให้ query ยาวเกินไป)
    if 'ascendant' in chart_info:
        ascendant = chart_info['ascendant']
        query += f" ลัคณาราศี{ascendant['sign']}"
